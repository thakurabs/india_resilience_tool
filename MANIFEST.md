...
| `prepare_jrc_rp100_source.py` | Prepare a planned JRC v2.1.2 RP-100 source inventory/manifest scaffold from a local official `tile_extents.geojson` or explicit tile filename list. Selects India-intersecting tiles from the canonical boundary union buffered by one native pixel (`1/1200°`), records boundary SHA-256 and official URL metadata, and writes `source_inventory.json` / planned `source_manifest.json` without downloading rasters; `README.md` documents the interim manual PowerShell bootstrap for downloading official RP-100 depth tiles, while validated tile download, raster validation, VRT creation, and coverage-mask export remain CHG-0232 work |
