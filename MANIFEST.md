...
| `powershell -ExecutionPolicy Bypass -File tools/runs/refresh_dashboard_climate_bundles.ps1 -State Telangana -Level all` | Refresh active admin dashboard climate bundles for district and block by computing only thematic and sector-wise source metrics, then rebuilding masters, composites, proposal bundles, optimized artifacts, and strict parity reports. Optional `-Bundle "<name>"` scopes the run to named dashboard bundle(s); compute is incremental (`--skip-existing`) by default with opt-in `-Overwrite`/`-OverwriteMetrics`; masters rebuild freshness-aware via compute completion markers; `-Workers` is opt-in; parity is audited once. Compute→master→composite run per-bundle with fail isolation (C2), then one union optimized+audit pass over the succeeded bundles; on any failure optimized+audit is skipped by default (non-zero exit) unless `-AllowPartialPublish` publishes the succeeded subset with a `*_partial_run.json` manifest |
...
| `python -m tools.pipeline.build_proposal_bundles --help` | Build persisted admin district/block proposal climate-risk bundle masters under `processed/<proposal_composite_slug>/<state>/` and the helper `r95p_interannual_variability` masters; Health Risk (lens dossier §6), Industrial Risk (CHG-0028, lens dossier §7), Investment / Financial Risk (CHG-0033, lens dossier §8), Infrastructure Risk (CHG-0034, lens dossier §9), Agricultural Risk (CHG-0032, lens dossier §12), Asset Risk (Hydropower Plants) (CHG-0036, lens dossier §11), and Life & Livelihood Loss Risk (CHG-0037, lens dossier §13) use explicit rule weights, lens-decomposed scoring, and an `available_rule_weight_fraction` coverage gate (0.70). Each rule persists `{rule_slug}__{scenario}__{period}__{score,abs_score,chg_score,imp_score}` columns under the four-token master schema, but only active lenses are written (for example Investment `r99p_positive_trend` omits `__imp_score`; see `docs/lens_scoring_methodology.md` §5.1). Per-lens rule columns are part of the `processed_optimised/` contract from artifact version 3 onward |
