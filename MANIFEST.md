...
| `refresh_dashboard_riverine_flood_bundle.ps1` | PowerShell operator script that refreshes the full dashboard-ready admin Riverine Flood bundle for one state by chaining `prepare_dashboard jrc-flood-depth`, district + block `build_composite_metrics` for `composite_flood_jrc_depth`, a state-scoped `build_processed_optimised` pass for the Riverine Flood metrics, and a strict state-scoped parity audit. Supports `-PlanOnly`, state-specific JRC builder path overrides, and optional `-IncludeSharedAdmin` when the operator intentionally wants shared admin artifacts rebuilt alongside the state-scoped refresh |
