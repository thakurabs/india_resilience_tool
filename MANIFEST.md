...
| `evapotranspiration.py` | Hargreaves-Samani PET and extraterrestrial-radiation arithmetic, unit conversion and aridity classes. Consumed by the P/PET path in `drought_risk_gridfirst.py`: monthly PET caching, annual precipitation/PET ratios, coverage-gated period means and admin aggregation. Registered as `aridity_index_p_over_pet`; not yet incorporated into the published drought composite. |
