...
| `python -m tools.diagnostics.wbgt_era5_grid_maps --dry-run` | Render the gridded WBGT comparison as spatial maps of the fields, the biases and their drivers (CHG-0551) |
...
| `wbgt_era5_grid_maps.py` | **Spatial renderer for the gridded WBGT comparison (CHG-0551).** Reads only the `per_cell.csv` written by `wbgt_era5_grid_compare.py` plus, optionally, `DATA_DIR/states_4326.geojson` for orientation; writes nothing under `IRT_DATA_DIR` and recomputes no WBGT — every number it draws comes from the comparison run, so the maps cannot drift from the scores they illustrate. Pivots the long CSV back onto the (lat, lon) grid and raises if more than one complete local day is present, rather than silently averaging days into a map that belongs to neither. Four figures: `fields.png` (the eight day-mean and day-peak WBGT fields on one shared scale), `deployed.png` (the two AS-DEPLOYED IRT fields beside their peak references), `bias.png` (the seven scored comparisons as candidate-minus-reference maps on a shared symmetric diverging scale, each panel annotated with its own mean and per-cell min/max because the formula-only panels are deliberately near-blank at the scale the aggregation panels demand) and `drivers.png` (Ta day-mean, Ta day-peak, the peak-minus-mean diurnal swing, day-mean RH, and a scatter of the swing against the as-deployed shade bias). The state overlay is optional by design: a missing or unreadable boundary layer prints a notice and draws without it, since the maps are data either way. `--dry-run` prints the read and write plan and renders nothing. Writes only under `--out-dir` (default `docs/diagnostics/wbgt_era5_grid`) |
