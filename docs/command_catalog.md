...
- the runner orchestrates:
  ...
  - `tools.pipeline.build_composite_metrics` for admin `district` and `block` climate runs
...
Composite stage notes:
- the composite stage is planned only for admin `district` and `block`
- hydro-only climate runs skip the composite stage
- admin climate runs place composite building after climate master generation and before optimized refresh
- composites are persisted for the 6 visible Glance bundles only
...
This command writes persisted district/block composite masters for the 6 visible Glance bundles under:
...
The corresponding optimized outputs are generated by the normal optimized runtime-bundle build under:
