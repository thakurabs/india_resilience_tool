...
**Status:** ✅ APPLIED to the note (2026-06-29) — the flattened §1 (CHG-0155–0160, incl. the 0158a bias-correction fix) was written into `technical_guidance_note.md` §1, replacing the old §1.1–§1.4. In-text author–year cites only; **the 8 full reference entries below are NOT yet in the note** (the note has no bibliography section — pending Q5 house-style decision). `GoI 2018` was harmonized to `Government of India 2018` on apply. Iterate further here as needed.

**Iteration round 3 (2026-06-29) — RBI report weaves (CHG-0155–0157, applied to this draft; note untouched):**
- Read *RBI Report on Currency and Finance 2022–23* (190 pp.) via pypdf in session scratch. Three weaves folded into the draft, all citing the verifiable synthesis/primary:
  - **CHG-0155 (¶3, multi-hazard):** added the RBI region→hazard-portfolio observation (Chart II.1) as an empirical anchor for "multiple and co-occurring" — cited **(RBI 2023)** only (dropped the secondary "drawing on Mani 2018 / Krishnan 2020" trail to avoid two non-load-bearing references).
  - **CHG-0156 (¶1, problem):** added the demand-side evidence — ~95% of surveyed institutions lack adequate climate-risk data + explicit call for a disaggregated-geographic physical-hazard database (RBI 2023, Ch. III survey).
  - **CHG-0157 (design-rationale ¶):** added the **Economic Survey 2017–18** district-percentile yield precedent (top-quintile temp → −4/−4.7% yield; bottom-quintile rainfall → −12.8%) as Indian-policy precedent for IRT's district-resolution, percentile framing — **(GoI 2018)**.
- New references added: **RBI (2023)** and **Government of India, Ministry of Finance (2018), Economic Survey 2017–18**. GoI (2018) exact string confirmed from the RBI bibliography (= *Economic Survey 2017-18*); the district-percentile regressions are in Vol. I, Ch. 6.
- Secondary RBI numbers (SWM 75% of rainfall; monsoon ↓~6–8%; +4.4 °C end-century; 4.5% GDP-at-risk by 2030) deliberately **not** imported — they pull toward impact-economics, outside the hazard-only scope.
- **CHG-0158 (¶2 reframe, user-flagged):** ¶2 previously led with "global models miss the monsoon → downscaling fixes it only partially," which read as evidence *against* using a downscaled product. Rebuilt (same facts) so downscaling is the affirmative reason for the product choice (it solves the resolution problem), and the residual monsoon uncertainty justifies the ensemble + relative-reading design rather than confessing weakness. The explicit "Two limitations / spatial resolution / regional fidelity" scaffolding is dropped.
- **CHG-0159 (out-of-scope sentence removed, user instruction):** deleted the "Out of scope are the web dashboard … tropical-cyclone and coastal-storm hazards, which are not yet represented" sentence from the scope paragraph. **Consequence to note:** the explicit cyclone "not yet represented" disclosure (round-2 fix for consistency risk #1) and the web-dashboard / groundwater scoping are now gone; exposure/vulnerability exclusion is still carried by the following IPCC-decomposition sentence ("because exposure and vulnerability are excluded …").
- **CHG-0158a (¶2 factual fix, user-flagged):** "bias-corrects against **an Indian observational record**" was **wrong** — NASA-NEX BCSD trains on **GMFD (Sheffield et al. 2006), a global dataset** (review-doc W4, already verified; note §3.2 correct). Changed to "a gridded observational record." The same slip in the round-2 guardrail (line 288) was also corrected. No Indian-specific bias-correction exists anywhere in the IRT pipeline.
- **CHG-0160 (¶4 DST naming, Q7 resolved):** named **DST (2021)** as the concrete referent for the "national … assessments" category and **dropped "exposure"** (DST maps *socio-economic vulnerability*, not exposure). Recast to "national common-framework assessments mapping socio-economic vulnerability at the district and state level (DST 2021)." Partially reverses C1 for this one item, by user decision.

**Iteration round 2 (2026-06-29) — resolved & folded into the draft above:**
- **Flattened §1** — removed `### 1.1–1.4`; §1 is now one continuous narrative (problem → why coarse products fail → multi-hazard/three-needs → landscape → design-choice bullets → contribution → scope → roadmap table → use/misuse). Only the bullet list and roadmap table remain as structural aids. The lone internal `(→ §1.3)` was dropped (self-referential once merged). No external cross-ref points into §1's subsections — verified `grep "§1\.[1-4]"` returns only the in-§1.4 line.
- **References added (user-approved: B1 propose-cites; A1–A3 web-verify; C1 soften):**
  - B1 critique → Dubash & Jogesh (2014) + Singh et al. (2017).
  - B2 policy anchor → **re-anchored**: the file the user supplied as "MoEFCC 2018" is actually **NAPCC 2008** (scanned; internal title `Inner 01-54-C 7.6.08`). So B2 now cites **Government of India (2008) NAPCC** (origin of SAPCCs) + **DST (2021) Common Framework** (verified from supplied PDF) — *not* a 2018 framework. The unverifiable "higher-resolution projections" clause was dropped.
  - A1 monsoon error → Konda & Vissa (2023). A2 IPCC risk → IPCC (2022) AR6 WGII. A3 SPI → McKee et al. (1993); ETCCDI → Zhang et al. (2011, already in note).
  - C1 landscape → **softened to unnamed categories** (no ND-GAIN/Aqueduct/DST/CEEW names, no citations needed).
- Two supplied PDFs read via pypdf installed into the session scratch (`scratchpad/pylibs`, `--target` — project env untouched). NAPCC is image-only (no text layer); DST front matter confirmed publisher/team/year.
...
- **Scientific precision on downscaling.** The draft does **not** claim statistical (BCSD) downscaling "fixes" monsoon dynamics — it credits downscaling for resolution + bias-correction to a **gridded observational reference (GMFD, Sheffield et al. 2006 — a *global* dataset, not Indian; see §3.2 / W4)**, and routes residual parent-GCM monsoon uncertainty through the ensemble/scenario spread. (A technical reader would catch an over-claim here — including the earlier "Indian observational reference" slip, now corrected.)
...
- **Q1 — Landscape citations.** ✅ RESOLVED — softened to unnamed categories; names + citations dropped.
- **Q2 — Policy-anchor depth.** ✅ RESOLVED — anchored on NAPCC (2008) + DST (2021), both from supplied PDFs; the "2019 MoEFCC framework" line is retired (supplied file was NAPCC 2008, not the 2018 framework).
- **Q3 — §1.1 length.** ✅ MOOT — subsections flattened; length is now judged on the whole §1 (≈10 paragraphs continuous). Open sub-question: if the continuous §1 reads heavy, candidate trims are the landscape paragraph (fold into the problem paragraph) or the monsoon-fidelity detail.
- **Q4 — CHG numbering.** STILL OPEN — reconcile CHG-0150–0154 against the 0145–0149 editorial batch before applying to the note.
- **Q5 (new) — Reference style.** The note needs a References section; format the 8 entries above in whatever house style the note adopts (currently inline DOIs in §2.3 etc.). Confirm style on apply.
- **Q6 (new) — DST/NAPCC exact citation strings.** DST commonly cited as 2021 (creation date Mar 2021) vs project years 2019–2020; NAPCC publisher as "Government of India" vs "MoEF". Pick on apply.
- **Q7 — ¶4 "national district-level assessments."** ✅ RESOLVED (CHG-0160) — named **DST (2021)**, dropped "exposure"; ¶4 now reads "national common-framework assessments mapping socio-economic vulnerability at the district and state level (DST 2021)." Partially reverses C1 for this one item, by user decision.
...
| ¶3 region→hazard-portfolio sentence (CHG-0155) | RBI 2023, Chart II.1 (p.36) | Authoritative Indian-institution statement that hazard exposure is regionally heterogeneous + multi-dimensional |
| ¶1 disaggregated-data demand (CHG-0156) | RBI 2023, Ch. III survey (III.25–26, p.~107) | Demand-side evidence the gap is recognised, not asserted; ~95% lack data + ask for disaggregated physical-hazard DB |
| Design-rationale district-percentile precedent (CHG-0157) | RBI 2023 ¶II.35 (p.~111) → primary GoI 2018 Economic Survey 2017-18 Ch.6 | Government analysis using the *same* district + relative-percentile logic IRT uses; methodological precedent |
...
The relevant hazards are also multiple and co-occurring: extreme daytime and night-time heat, humid heat, meteorological drought, extreme rainfall and flash flooding, winter cold, and riverine flooding, each with its own seasonality and geography. This regional patterning is well documented: a recent survey of India's physiographic zones assigns each a distinct hazard portfolio — heatwaves over the arid northwest, landslides and cloudbursts across the Himalaya, river floods and heat over the Indo-Gangetic plains, cyclones and urban flooding along the coasts and the Ghats, and drought, heat, and forest fire over the central peninsular plateau (RBI 2023). Adaptation decisions against these hazards are made for multi-decade horizons under genuine scenario and model uncertainty, so planners need projections that are resolved to the administrative units they manage, available for more than one emissions future, and available for more than one time horizon. IRT is organised around exactly those three needs.

Several existing efforts map climate risk in or over India at particular scales or scopes — global readiness-and-vulnerability indices at country resolution, single-risk tools focused on water stress, and national common-framework assessments mapping socio-economic vulnerability at the district and state level (DST 2021). IRT is complementary rather than competing: it contributes a multi-hazard, downscaled, per-period reading of the climate **hazard pressure** itself, at district and block level, intended to sit alongside the exposure and vulnerability layers those efforts provide.

Concretely, the tool's design choices, stated plainly:
...
- **Hazards and indices.** A library of standard climate indices (ETCCDI — Zhang et al. 2011; SPI — McKee et al. 1993) and India-context indices spanning heat, humid heat, cold, drought, and extreme rainfall, plus a static riverine-flood layer from global flood-hazard data (→ §5).
...
IRT's contribution is to operate at the administrative resolution — district (ADM2) and block (ADM3) — at which Indian adaptation is actually planned and which current national guidance now asks states to reach, using India-context indices and thresholds and explicit per-hazard decomposition rather than a single aggregate index value. This unit and framing have precedent in Indian policy analysis: the Government of India's *Economic Survey 2017–18* quantified climate impacts using district-level percentile thresholds, finding that a district in the top quintile of the temperature distribution loses on the order of 4–4.7 per cent of crop yield, and one in the bottom quintile of rainfall as much as 12.8 per cent (GoI 2018) — the same district-resolution, relative-percentile logic IRT applies to hazard exposure.
...
### References (new — to be merged into the note's reference section on apply)

- Dubash, N.K. & Jogesh, A. (2014). *From Margins to Mainstream? State Climate Change Planning in India.* Centre for Policy Research, New Delhi.
- Government of India (2008). *National Action Plan on Climate Change.* Prime Minister's Council on Climate Change, New Delhi. *(supplied PDF — scanned; internal title `Inner 01-54-C 7.6.08`, 7 June 2008)*
- Department of Science and Technology (2021). *Climate Vulnerability Assessment for Adaptation Planning in India Using a Common Framework.* DST, Government of India — IIT Mandi, IIT Guwahati & IISc Bengaluru (project 2019–2020). *(supplied PDF, verified)*
- Government of India, Ministry of Finance (2018). *Economic Survey 2017–18, Volume I*, Chapter 6: "Climate, Climate Change, and Agriculture: Coping with Climate Change." Department of Economic Affairs, New Delhi. *(district-percentile yield finding; cited via RBI 2023 and confirmed against the primary)*
- Reserve Bank of India (2023). *Report on Currency and Finance 2022–23: Towards a Greener Cleaner India.* RBI, Mumbai. *(region→hazard-portfolio synthesis, Chart II.1; disaggregated-data demand, Ch. III stakeholder survey)*
- Singh, C., Deshpande, T. & Basu, R. (2017). How do we assess vulnerability to climate change in India? A systematic review of literature. *Regional Environmental Change*, 17(2), 527–538. DOI 10.1007/s10113-016-1043-y.
- Konda, G. & Vissa, N.K. (2023). Evaluation of CMIP6 models for simulations of surplus/deficit summer monsoon conditions over India. *Climate Dynamics*, 60, 1023–1042. DOI 10.1007/s00382-022-06367-1.
- IPCC (2022). *Summary for Policymakers.* In *Climate Change 2022: Impacts, Adaptation and Vulnerability* (WGII contribution to AR6). Cambridge University Press.
- McKee, T.B., Doesken, N.J. & Kleist, J. (1993). The relationship of drought frequency and duration to time scales. *Proc. 8th Conf. on Applied Climatology*, AMS, Boston, 179–184.
- Zhang, X., Alexander, L., Hegerl, G.C., et al. (2011). Indices for monitoring changes in extremes based on daily temperature and precipitation data. *WIREs Climate Change*, 2(6), 851–870. *(already cited in the note)*
