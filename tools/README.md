...
| `tools/data_acquisition/prepare_jrc_rp100_source.py` | Prepare a planned JRC v2.1.2 RP-100 source inventory/manifest scaffold from a local `tile_extents.geojson` or explicit tile filename list; selects India-intersecting tiles with the one-native-pixel buffer and does not download rasters in CHG-0231. See the JRC flood-depth section in `README.md` for the interim manual PowerShell download bootstrap until CHG-0232 adds the validated downloader | `python -m tools.data_acquisition.prepare_jrc_rp100_source --help` |
