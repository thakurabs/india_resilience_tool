...
| `tools/diagnostics/wbgt_era5_grid_maps.py` | **Read-only** renderer for the gridded WBGT comparison (CHG-0551): reads the `per_cell.csv` written by `wbgt_era5_grid_compare.py` and draws four PNG figures. `fields.png` puts all eight WBGT fields (IRT shade, IRT sWBGT, Bernard, Liljegren; each as day-mean and day-peak) on one shared colour scale so the level gap between the shade and outdoor families is readable off the panels rather than only from the score table. `deployed.png` sets the two AS-DEPLOYED IRT fields beside their peak references. `bias.png` maps the seven scored comparisons on a shared symmetric diverging scale — the formula-only panels come out near-blank against the aggregation panels, which is the finding, so each panel is annotated with its own mean and per-cell range. `drivers.png` maps Ta day-mean, Ta day-peak, the peak-minus-mean diurnal swing and day-mean RH, and adds a scatter of that swing against the as-deployed shade bias. State outlines are an optional overlay read from `DATA_DIR/states_4326.geojson`; a missing layer degrades to no outline rather than failing the run. Raises if `per_cell.csv` holds more than one complete local day rather than averaging across days. `--dry-run` prints the read and write plan and renders nothing. Writes only under `--out-dir` (default `docs/diagnostics/wbgt_era5_grid`). | `python -m tools.diagnostics.wbgt_era5_grid_maps --dry-run` |
