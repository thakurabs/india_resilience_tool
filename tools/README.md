...
> **⚠️ `jrc-flood-depth --overwrite` caveat.** The `jrc-flood-depth` plan currently
> schedules the `blocks-geojson` step, whose QA writer refuses to overwrite without
> `--overwrite`. So on a machine where the canonical blocks already exist, the
> **no-`--overwrite`** run fails fast at step 1 (`FileExistsError` on
> `block_boundary_repair_summary.csv`), while adding **`--overwrite`** silently
> **rebuilds the pipeline-wide canonical blocks GeoJSON** (`blocks_4326.geojson`,
> all 36 states) + its QA CSVs as a side-effect — a global artifact regenerated by
> what looks like a single-state JRC run. The rebuild is deterministic from the
> source shapefile, so content is normally unchanged, but it is a broader blast
> radius than a JRC run should have. The blocks step is unnecessary here because the
> JRC builder only *reads* the canonical blocks file.
>
> Until the planner is fixed (deferred CHG-0065: skip `blocks-geojson` when the
> canonical file already exists), if you must use `--overwrite`, know that it
> touches the shared blocks GeoJSON. To build JRC masters without that side-effect,
> run the builder directly (`python -m tools.geodata.build_jrc_flood_depth_admin_masters
> --state <State> --source-dir <dir> --assume-units m`) and publish separately, or
> apply CHG-0065 first.
