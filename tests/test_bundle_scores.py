...
def test_compute_bundle_score_frame_averages_normalized_metrics_not_raw_values() -> None:
    ...
    out = compute_bundle_score_frame(
        df,
        metric_specs=[
            BundleMetricSpec(slug="heat_days", label="Heat days", column="heat_days", higher_is_worse=True),
            BundleMetricSpec(slug="warm_nights", label="Warm nights", column="warm_nights", higher_is_worse=True),
        ],
